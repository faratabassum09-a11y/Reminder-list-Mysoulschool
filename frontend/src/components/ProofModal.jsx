...
// "Mark as done" — completes the task immediately, no admin approval step.
// Proof (message/link/screenshot) is optional, kept as a record for the
// admin to glance at from the Notifications page.
export default function ProofModal({ row, onClose, onChanged }) {
  ...
  const submit = async (e) => {
    ...
    setError("");
    try {
      await api.submitDone(row._id, form);
      toast("Marked as done — the admin's been notified", "good");
      onChanged();
      onClose();
    } catch (err) {
    ...
    <div className="modal-scrim" onMouseDown={(e) => e.target === e.currentTarget && onClose()}>
      <div className="modal" role="dialog" aria-modal="true">
        ...
        <form onSubmit={submit}>
          ...
          <p className="form-hint">Everything here is optional. Marking as done completes the task right away and notifies the admin.</p>
          {error && <p className="error">{error}</p>}
          <div className="modal-actions">
            <button type="button" className="btn-ghost" onClick={onClose}>Cancel</button>
            <button type="submit" disabled={busy}>{busy ? "Marking…" : "✔ Mark as done"}</button>
          </div>
        </form>
      </div>
    </div>
